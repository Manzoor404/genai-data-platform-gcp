...
def create_spark():
    return SparkSession.builder.appName("curate_customers").getOrCreate()
...
def read_processed(spark, config):
    ...
    table = config["bq"]["customers_table"]

    return spark.read.format("bigquery").option(
...
def transform(df):
    ...
    return (
        df.select(
            ...
            lit("processed_layer.customers").alias("record_source"),
...
def write(df, config):
    ...
    table = config["bq"]["curated_customers_table"]
...
def main():
    ...
    print("========== CURATED SUMMARY: CUSTOMERS ==========")
