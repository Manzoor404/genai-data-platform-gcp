...
def create_spark():
    return SparkSession.builder.appName("curate_category_translation").getOrCreate()


def read_processed(spark, config):
    project = config["project_id"]
    dataset = config["bq"]["processed_dataset"]
    table = config["bq"]["category_translation_table"]
...
def transform(df):
    return (
        df.select(
            ...
            lit("processed_layer.category_translation").alias("record_source"),
...
def write(df, config):
    ...
    table = config["bq"]["curated_category_translation_table"]
...
def main():
    ...
    print("========== CURATED SUMMARY: CATEGORY TRANSLATION ==========")
