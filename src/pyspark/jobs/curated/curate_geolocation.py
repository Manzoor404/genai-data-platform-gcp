...
def create_spark():
    return SparkSession.builder.appName("curate_geolocation").getOrCreate()


def read_processed(spark, config):
    project = config["project_id"]
    dataset = config["bq"]["processed_dataset"]
    table = config["bq"]["geolocation_table"]
...
def transform(df):
    return (
        df.select(
            ...
            col("geolocation_lat"),
            col("geolocation_lng"),
            trim(col("geolocation_city")).alias("geolocation_city"),
            trim(col("geolocation_state")).alias("geolocation_state"),
            lit("processed_layer.geolocation").alias("record_source"),
...
def write(df, config):
    ...
    table = config["bq"]["curated_geolocation_table"]
...
def main():
    ...
    print("========== CURATED SUMMARY: GEOLOCATION ==========")
